...
pref0admintoall = True   # Разрешить всем выключать сервер
pref0showall = False     # Показывать ли содержимое сообщений в консоли сервера

# --- Кроссплатформенное определение папки данных ---
if platform.system() == "Windows":
    APPDATAAHH = os.getenv("APPDATA")
    if APPDATAAHH is None:
        APPDATAAHH = os.getcwd()
else:
    # На Linux (Render) храним рядом с проектом, а не в корневой папке
    APPDATAAHH = os.getcwd()
...
# --- Windows Tray Functions (работают только на Windows) ---
if platform.system() == "Windows":
...
# --- ИСПРАВЛЕННЫЙ ОБРАБОТЧИК ПОДКЛЮЧЕНИЯ ---
@s.on("join")
def usrn(name):
    # Теперь мы просто принимаем строку с именем
...
def chat(msg):
    ...
    mainchat[:] = mainchat[-20:]
    emit("mainchat", d, broadcast=True)
    
    if pref0showall:
        tempmessageshow = f'"{msg}"'
    else:
        tempmessageshow = "(hidden)"
...
if __name__ == '__main__':
    # Запускаем иконку только на Windows
    if platform.system() == "Windows":
        if consolepi():
            hideall()
